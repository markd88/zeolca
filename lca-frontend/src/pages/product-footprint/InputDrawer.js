...
const App = (props) => {
  ...
  params.newInputRef.current = {}

  params.newInputRef.current.reset = form_reset
  ...
  const setFactor = (factor_info) => {
    if (factor_id !== "factor_db_basic") {
      if (factor_info.second !== "t·km") {
        alert(" the unit does not match, here")
      }
      else {
  ...
  const onFinish = (values) => {
    ...
    if (!basicFactorDisabled) {
      ...
      console.log(values.factor_db_basic, input_unit)
    ...
    if (! unit_match) {
      ...
      console.log(values.factor_db_basic)
      ...
      .then((response) => {
        ...
        if (response.data.status === 0) {
          alert("create new input/output success")
          params.inputClose()
          params.setTab("2")
